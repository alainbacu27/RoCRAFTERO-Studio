...
local enablePlaceIconOnLoadingScreenSuccess, enablePlaceIconOnLoadingScreenValue = pcall(function() return settings():GetFFlag("EnablePlaceIconOnLoadingScreen") end)
local enablePlaceIconOnLoadingScreen = enablePlaceIconOnLoadingScreenSuccess and enablePlaceIconOnLoadingScreenValue
	...
	if debugMode then
		...
		if enableNewLoadingScreen then
			...
			if enableNewLoadingScreen then
				...
				local function fadeIconSpinner()
					if backgroundFadeStarted then
						return
					end
					...
					while now - startTime < fadeDuration do
						--Bail out if the background fade started or we'll start fighting with it
						if backgroundFadeStarted then
							return
						end
						...
						--t is linearly 0-1 over the duration of the fade
						local t = ((now - startTime) / fadeDuration)

						placeIcon.ImageTransparency = (1 - t) ^ 2

						t = -(t^2) + (2*t)
						placeIconAspect.AspectRatio = t * (576 / 324)
					end
