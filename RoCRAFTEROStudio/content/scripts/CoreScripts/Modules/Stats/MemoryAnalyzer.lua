...
function __GetMemoryPerformanceStatsItem()
  ...
  local memoryStats = performanceStats:FindFirstChild(
      StatsUtils.StatNames[StatsUtils.StatType_PlaceMemory])          
  return memoryStats
end
...
function __GetMemoryTypeNameValueTriplets()
  ...
  if (StatsUtils.FFlagMemoryTrackerInPerformanceStats) then 
    table.insert(retVal, {"Total", "Total", memoryStats:GetValue()})
  end
  
  local statsItems = memoryStats:GetChildren()
  for i, statsItem in ipairs(statsItems) do
    ...
    table.insert(retVal, {statType, statName, statValue})
  end
...
function MemoryAnalyzerClass:__getOrMakeRowForStatType(statType, statLabel) 
...
function MemoryAnalyzerClass:__updateStatValue(statType, statLabel, value) 
...
function MemoryAnalyzerClass:renderUpdates()
  local typeValuePairs = __GetMemoryTypeNameValueTriplets()
  for i, triplet in ipairs(typeValuePairs) do
